...
import { WatchableObject } from "../watchable/watch-object";
...
export class GroupObject extends ViewObject {
    ...
    public GetListIcon: string = Icons.folder;
    ...
    public Add(m3u: M3UObject): WatchableObject {
        if (m3u.checkTypeIfTvShow()) {
            return this.AddTvShow(m3u);
        }
    ...
    public AddTvShow(m3u: M3UObject): WatchableObject {
        const tvShowGroup = this.addTvGroup(m3u.tvShowName);
        const tvShowSeazon = tvShowGroup.addSeason(m3u.tvShowSeason);
